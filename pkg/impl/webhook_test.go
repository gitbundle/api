...
func TestWebhooks(t *testing.T) {
	tests := []struct {
		...
		// pull request comment hooks
		{
			...
			obj:    new(api.PullRequestCommentHook),
		},
	}
	...
	for _, test := range tests {
		t.Run(test.before, func(t *testing.T) {
			t.Run(test.before, func(t *testing.T) {
				...
				if err != nil && err != api.ErrSignatureInvalid {
					t.Error(err)
					return
				}
...
func TestWebhookInvalid(t *testing.T) {
	...
	s := new(webhookService)
	_, err := s.Parse(r, secretFunc)
	if err != api.ErrSignatureInvalid {
...
func secretFunc(api.Webhook) (string, error) {
