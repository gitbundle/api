...
import (
	...
	"github.com/gitbundle/api/pkg/impl"
)
...
func ExampleClient() {
	client, err := impl.New("https://api.api.com")
	if err != nil {
		log.Fatal(err)
	}

	// Sets a custom http.Client. This can be used with
	// api.com/golang/oauth2 for authorization.
...
func ExampleWebhook() {
	client := impl.NewDefault()

	secret := func(webhook api.Webhook) (string, error) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		case *api.TagHook:
		case *api.IssueHook:
		case *api.IssueCommentHook:
		case *api.PullRequestHook:
		case *api.PullRequestCommentHook:
		case *api.ReviewCommentHook:
		}
	}
...
func ExampleWebhook_lookupSecret() {
	...
	secret := func(webhook api.Webhook) (secret string, err error) {
